...
public:
	...
	virtual void set_percent(double percent) = 0;
	virtual double get_percent() = 0;
	friend class Debet;
	friend class Credit;
};

class Debet : public Account {
	double percents;
public:
	Debet() {
		money = 0;
	...
	void percent() {
		setMoney(this->getMoney()*(1+percents / 100));
	}
...
public:
	Credit() {
		money = 0;
		//const char* firstname = "123";
		//const char* lastname = "343";

		//AccountHolder p(firstname, lastname);
		//setAccountHolder(p);
		number = 0;
		percents = 0;
	}
...
int main() {
	...
	cout << new1;*/
	srand(time(NULL));
	Account* list[3] = { new Debet(), new Credit(), new Debet()};
	for (int i = 0; i < 3; i++) {
		list[i]->setMoney(rand()%1000);
		list[i]->set_percent(rand() % 100);
		cout << i << ": " << list[i]->getMoney() << "   " << list[i]->get_percent() << endl;
	}
	for (int i = 0; i < 3; i++) {
		list[i]->percent();
		cout << i << ": " << list[i]->getMoney() << "   " << list[i]->get_percent() << endl;
